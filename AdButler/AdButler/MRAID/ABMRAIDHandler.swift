...
public class ABMRAIDHandler : NSObject, WKUIDelegate, WKNavigationDelegate {
    private var mraidDelegate:ABMRAIDDelegate? = nil
    ...
    public func initialize(parentViewController:UIViewController, mraidDelegate:ABMRAIDDelegate){
        ...
        if(url != nil && url!.absoluteString != "about:blank"){
            if(
                url!.absoluteString.range(of:"servedbyadbutler.com") != nil ||
                    url!.absoluteString.range(of:"adbutler-fermion.com") != nil
                ){
    ...
    public func addCloseButton(to:UIView, action:Selector, showButton:Bool, position:String = "top-right"){
        ...
        if(showButton){ // resized views will create a close button region, but not display it as per MRAID spec
            ...
            closeButton.setBackgroundImage(UIImage(named:"closeButtonBG", in: Bundle(identifier:"sparklit.AdButler"), compatibleWith:nil), for: UIControl.State.normal)
...
private func setMRAIDCurrentPosition(_ pos:CGRect){
    ...
    private func setOrientationProperties(_ args:String?){
        ...
        do{
            if(args != nil){
                var properties:OrientationProperties? = try ABMRAIDUtilities.deserialize(args!)
                if(properties != nil){
    ...
    public func setResizeProperties(_ args:String?){
        ...
        do{
            if(args != nil){
                let properties:ResizeProperties? = try ABMRAIDUtilities.deserialize(args!)
                if(properties != nil){
    ...
    public func setExpandProperties(_ args:String?){
        ...
        do{
            if(args != nil){
                let properties:ExpandProperties? = try ABMRAIDUtilities.deserialize(args!)
                if(properties != nil){
    ...
    private func saveCalendarEvent(eventStr:String, completionHandler:@escaping ()->Void){
        do{
            let mraidEvent:MRAIDCalendarEvent? = try ABMRAIDUtilities.deserialize(eventStr)
            if(mraidEvent != nil){
                let status = EKEventStore.authorizationStatus(for: EKEntityType.event)
                if(status == .authorized){
                    let success = ABMRAIDUtilities.addCalendarItem(mraidEvent: mraidEvent!)
                    if(success){
                        completionHandler()
                    }else{
