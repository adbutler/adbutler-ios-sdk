...
//  AdButler
//
//  Created by Will Prevett on 2018-07-25.
...
public class ABMRAIDBanner: UIViewController, ABMRAIDDelegate {
    ...
    private func initialize(placement:Placement, parentViewController:UIViewController, position:String?, frame:CGRect?, respectSafeArea:Bool = false){
        ...
        if(placement.body != nil){
            ...
            initBanner()
            ABMRAIDUtilities.validateHTML(&renderBody)
            
            let url = Bundle(identifier:"sparklit.AdButler")?.bundleURL
    ...
    private func initSecondaryWebView(_ url:String){
        ...
        ABMRAIDUtilities.getExpandedUrlContent(url, completion: { val in
    ...
    private func setResizeMask(){
        ...
        if(layoutPosition!.range(of:"bottom") != nil){
            if(mraidHandler.respectsSafeArea){
                if #available(iOS 11.0, *) {
                    let guide = parentController.view.safeAreaLayoutGuide
                    NSLayoutConstraint.activate([view.bottomAnchor.constraint(equalToSystemSpacingBelow: guide.bottomAnchor, multiplier: 1.0)])
                } else {
    ...
    public func reportDOMSize(_ args:String?){
        do{
            if(defaultSize == nil && args != nil){
                let size:Size? = try ABMRAIDUtilities.deserialize(args!)
                if(size != nil){
                    if(size!.width == 0 || size!.height == 0){
                        defaultSize = fallbackSize
                    }else{
